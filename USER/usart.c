...
* Call: internal
*/
void PC_String_Reciever(void)
{
	uint8_t i=0;
	uint8_t Rec_Buffer[20];

  if(USART_GetFlagStatus(PC_COM, USART_IT_RXNE) == SET){
		if(USART_ReceiveData(PC_COM)=='\n'){
			USART_RDataCnt = 0;
			USART_CMD_FLAG = SET;
			for(i=0;i<20;i++)Rec_Buffer[i]='\0';
		}
		else{
			if(USART_RDataCnt==0){
				for(i=0;i<20;i++)SendData[i]='\0';
			}
			Rec_Buffer[USART_RDataCnt]=USART_ReceiveData(PC_COM);
			SendData[USART_RDataCnt]=Rec_Buffer[USART_RDataCnt];
			USART_RDataCnt++;
		}
	}
