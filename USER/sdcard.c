...
static void SD_DMA_TxConfig(uint16_t *BufferSRC, uint32_t BufferSize);
...
{
    return (uint32_t)DMA_GetFlagStatus(DMA2_Stream3, DMA_FLAG_TCIF3);     //Channel4 transfer complete flag. 
}
...
void SD_DMA_RxConfig(uint32_t *BufferDST, uint32_t BufferSize)
{
    ...
    DMA_ClearFlag(DMA2_Stream3,DMA_FLAG_FEIF3|DMA_FLAG_TCIF3 | DMA_FLAG_TEIF3 | DMA_FLAG_HTIF3 | DMA_FLAG_DMEIF3);//清除DMA标志位

    /*!< DMA2 Channel4 disable */
    DMA_Cmd(DMA2_Stream3, DISABLE);        //SDIO为第四通道
    DMA_DeInit(DMA2_Stream3);
    /*!< DMA2 Channel4 Config */
    DMA_InitStructure.DMA_Channel=DMA_Channel_4;
    DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)SDIO_FIFO_ADDRESS;    //外设地址，fifo
    DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)BufferDST; //目标地址
    DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;        //外设为原地址
    ...
    DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_HalfWord;        //外设数据大小为字，32位
...
//    DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral;//外设为写入目标
...
//    DMA_Init(DMA2_Stream3, &DMA_InitStructure);
//    DMA_FlowControllerConfig(DMA2_Stream3, DMA_FlowCtrl_Peripheral);
//    //DMA_ITConfig(DMA2_Stream3,DMA_IT_FE,ENABLE);
//    /*!< DMA2 Channel4 enable */                             //不设置dma中断
//    DMA_Cmd(DMA2_Stream3, ENABLE);     
//}

void SD_DMA_TxConfig(uint16_t *BufferSRC, uint32_t BufferSize)
{
    DMA_InitTypeDef DMA_InitStructure;
...
SD_Error SD_Init(void)
{
    ...
    GPIO_SDIO_Configuration();/*!< SDIO 外设底层引脚初始化 */

    SDIO_DeInit();/*!<对SDIO的所有寄存器进行复位*/

    errorstatus = SD_PowerON(); /*!<上电并进行卡识别流程，确认卡的操作电压    */
        
    if (errorstatus != SD_OK)/*!< CMD Response TimeOut (wait for CMDSENT flag) */
    {
#if SDDEBUG
        printf("\r\n SD_PowerOn Fail,errorstatus is %d",errorstatus);
#endif
        return(errorstatus);        
    }
    else
    ...
    {    
#if SDDEBUG
        printf("\r\n SD_Initialize Fail,errorstatus is %d",errorstatus);
#endif
        return(errorstatus);        
    }
    else
    ...
    SD_GetCardInfo(&SDCardInfo);//用来读取csd/cid寄存器

    if (errorstatus == SD_OK)
    {
        /*!<----------------- Select Card --------------------------------*/
        errorstatus = SD_SelectDeselect((uint32_t) (SDCardInfo.RCA << 16));            //通过cmd7    ,rca选择要操作的卡
        if (SDCardInfo.SDCardLockStatus==LOCKED) //检测卡是否已上锁
        {
...
/*!< CMD8: SEND_IF_COND ----------------------------------------------------*/
 ...
 or SD card 1.x */
    if (errorstatus == SD_OK)        //响应了cmd55，是sd卡，可能为1.x,可能为2.0
    {
        ...
        if (response &= SD_HIGH_CAPACITY)    /*!<检查卡返回信息中的HCS位*/
        {
...
SD_Error SD_ReadMultiBlocks(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
    ...
    /*!< Set Block Size for Card (Only For SDSC) */
    SDIO_CmdInitStructure.SDIO_Argument = (uint32_t) BlockSize;
    SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SET_BLOCKLEN;
    SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;     
    SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
...
#elif defined (SD_DMA_MODE)        
    SDIO_ITConfig(SDIO_IT_DATAEND, ENABLE);        
    SD_DMA_TxConfig((uint16_t *)writebuff, BlockSize); 
    SDIO_DMACmd(ENABLE);
#endif
...
SD_Error SD_WriteMultiBlocks(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
    ...
    SDIO_ITConfig(SDIO_IT_DATAEND, ENABLE);         
    SD_DMA_TxConfig((uint16_t *)writebuff, (NumberOfBlocks * BlockSize));
    SDIO_DMACmd(ENABLE);         
...
SD_Error SD_WaitWriteOperation(void)
{
    ...
    while ((SD_DMAEndOfTransferStatus() == RESET)&&(TransferEnd == 0) && (TransferError == SD_OK))/*!<(SD_DMAEndOfTransferStatus() == RESET) */
    {
    	if(SDIO_GetFlagStatus(SDIO_FLAG_STBITERR))
			return SD_START_BIT_ERR;
    }
...
SD_Error SD_Erase(uint32_t startaddr, uint32_t endaddr)
{
    ...
    if ( SDCardInfo.SDCardLockStatus==LOCKED )            //卡已上锁
    {
        errorstatus = SD_LOCK_UNLOCK_FAILED;
        return(errorstatus);
    }
...
{
    if (StopCondition == 1)    /*!<Multi Block need Stop Command*/
    {
...
void DMA_ProcessIRQSrc(void)
{
    printf("\r\n An error of DMA occured!" );
...
static SD_Error CmdResp6Error(uint8_t cmd, uint16_t *prca)
{
    ...
    if( response_r1 & SD_CARD_LOCKED)
        SDCardInfo.SDCardLockStatus= LOCKED;
    else
        SDCardInfo.SDCardLockStatus = UNLOCK;
...
#if SDDEBUG
    printf("\r\n FindSCR,errorstatus is %d",errorstatus);
    printf("\r\n SCR[0]=%d,SCR[1]=%d",scr[0],scr[1]);
#endif
...
{ 
    ...
    errorstatus = CmdResp1Error(SD_CMD_SET_BLOCKLEN);
#if SDDEBUG
    printf("\r\n SET_BLOCKLEN,Errorstatus is %d",errorstatus);
#endif
    if (errorstatus != SD_OK)
    {
        return(errorstatus);
    ...
    errorstatus = CmdResp1Error(SD_CMD_APP_CMD);
#if SDDEBUG
    printf("\r\n APP_CMD,Errorstatus is %d",errorstatus);
#endif
    if (errorstatus != SD_OK)
    {
        return(errorstatus);
...
#if SDDEBUG
    printf("\r\n APP_SEND_SCR,Errorstatus is %d",errorstatus);
#endif
    if (errorstatus != SD_OK)
    {
        return(errorstatus);
    ...
    status = SDIO->STA;
    while (!(status & (SDIO_FLAG_RXOVERR |     
                    SDIO_FLAG_DCRCFAIL |
                    SDIO_FLAG_DTIMEOUT |
                    SDIO_FLAG_STBITERR 
                    )))
...
#if SDDEBUG
    printf("\r\n READ SCR,Status is 0x%x",status);
#endif
...
/*!<*
    * @brief    SDIO,DMA中断配置.
    * @param    NumberOfBytes: None.
    * @retval None
    */
void SDDMA_VIC_Init(void)
{
